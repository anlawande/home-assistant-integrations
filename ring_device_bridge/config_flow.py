...
from . import async_discover_devices, RingDeviceException
from .const import DOMAIN
from .ring import RingDevice, Discover


class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for ring_device_bridge."""

    VERSION = 1

    def __init__(self) -> None:
        """Initialize the config flow."""
        self._discovered_devices: dict[str, RingDevice] = {}
        self._discovered_device: RingDevice | None = None
    ...
    async def _async_handle_discovery(self, host: str, mac: str) -> FlowResult:
        ...
        try:
            self._discovered_device = await self._async_try_connect(
                host, mac, raise_on_progress=True
            )
        except RingDeviceException:
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                device = await self._async_try_connect(host, user_input[CONF_MAC], raise_on_progress=False)
            except RingDeviceException:
    ...
    @callback
    def _async_create_entry_from_device(self, device: RingDevice) -> FlowResult:
    ...
    async def _async_try_connect(
        self, host: str, mac: str, raise_on_progress: bool = True
    ) -> RingDevice:
        """Try to connect."""
        self._async_abort_entries_match({CONF_HOST: host})
        device: RingDevice = await Discover.discover_single(mac)
